...
class MessageProcedure:
    ...
    def encrypt_part(self, part_plain_text, this_part, num_parts):                
        ...
        self._machine.set_rotor_positions(indicator_inputs[indicatorprocessor.MESSAGE_KEY])
    ...
    def decrypt_part(self, cipher_text_part):
        ...
        self._machine.set_rotor_positions(indicators[indicatorprocessor.MESSAGE_KEY]) # Set message key        
        
        if self._step_before_proc:
            self._machine.step()            
        
        # Use message length to strip padding off at the end of the message body
        if formatter.MESSAGE_LENGTH in indicators.keys():
            ciphertext = help.text[:indicators[formatter.MESSAGE_LENGTH]]
...
class SG39IndicatorHelper:
    ...
    def test(self, indicator_candidate):
        result = indicatorprocessor.MsgKeyTestResult(False, indicator_candidate[:4])
...
class MessageProcedureFactory:
    ...
    def _get_incomplete_enigma(self, num_rotors = 3):
        ...
        result.formatter = formatter.EnigmaFormatter(num_rotors)
        result.formatter.limits = (5, 10)
        result.msg_size = 245
        result.encoder = transportencoder.ArmyEncoder()
    ...
    def get_post1940_enigma(self, system_indicator, grundstellung, num_rotors = 3):    
        ...
        result.indicator_proc = indicatorprocessor.Post1940EnigmaIndicatorProc(self._server, self._rand_gen, self.get_and_test_kenngruppen(system_indicator), num_rotors)
    ...
    def get_pre1940_enigma(self, system_indicator, grundstellung, num_rotors = 3):    
        result = self._get_incomplete_enigma(num_rotors)
        result.indicator_proc = indicatorprocessor.Pre1940EnigmaIndicatorProc(self._server, self._rand_gen, self.get_and_test_kenngruppen(system_indicator), grundstellung, num_rotors)
    ...
    def get_generic_machine(self, system_indicator, grundstellung, indicator_group_size, step_before_use = False):
        ...
        result.indicator_proc = indicatorprocessor.GrundstellungIndicatorProc(self._server, self._rand_gen, indicator_group_size, step_before_use)
        result.indicator_proc.grundstellung = grundstellung
        result.formatter = formatter.GenericFormatter(1, indicator_group_size, result.indicator_proc.key_words)
    ...
    def get_generic_kl7(self, system_indicator, grundstellung):
        ...
        result.encoder = transportencoder.KL7Encoder()
    ...
    def get_sigaba_basic(self, system_indicator, grundstellung):
        result = MessageProcedure(self._machine, self._rand_gen, self._server)
        result.indicator_proc = indicatorprocessor.SIGABABasicIndicatorProcessor(self._server, self._rand_gen)
        result.formatter = formatter.SIGABAFormatter()
        result.formatter.external_indicator = system_indicator
        result.formatter.limits = (5, 10)
        result.msg_size = 1730
        result.encoder = transportencoder.SIGABAEncoder()
    ...
    def get_sigaba_grundstellung(self, system_indicator, grundstellung):
        ...
        result.indicator_proc = indicatorprocessor.SIGABAGrundstellungIndicatorProcessor(self._server, self._rand_gen)
        result.indicator_proc.grundstellung = grundstellung
