...
class TestArithmetic(TlvProxy):
    ...
    def massen_test_add(self, num_test):
...
def with_test(left_summand, right_summand, echo_arg = TlvEntry().to_sequence(get_test_sequence())):
    with TlvServer('./tlv_object', 'sock_tmpjffdfkdfgj') as s:
        with TestArithmetic(s.address) as a, TestEcho(s.address) as e:
            print(a.add(left_summand, right_summand))
            print(e.echo(echo_arg))
            print(s.list_objects())
            print(s.list_providers())
            a.massen_test_add(22000)
        
def without_test():
    server = TlvServer(server_address = 'sock_tmpsuuerzbi')
    server.start()
    a = TestArithmetic(server.address)
    print(a.add(5, 7))
    server.stop()
